...
Console.WriteLine("GyWare WSCC VBScript to JavaScript Converter [https://github.com/gyware/wscc]")
...
var convert=function(){
	...
	var fileStream = File.OpenText(Environment.GetCommandLineArgs()[1])
	var strRet = fileStream.ReadToEnd();
	fileStream.Close();

	// choose right conversion type
	var conversionType = 1;
	if(conversionType==1){	// vbs to js
		...
		var strCommentText = "// ";
	}else if(conversionType==2){	//js to vbs
...
*/
File.WriteAllText(Path.ChangeExtension(Environment.GetCommandLineArgs()[1], ".js"), strRet);
}
...
function vb2js_functionreturn(p_strFunction){
	...
	var match = re.exec(p_strFunction);
	var strFunctionName = match[1];
...
function getVBStoJSOptionsArr(){
	...
	var collapseConcat = false;
	if(collapseConcat == true){
